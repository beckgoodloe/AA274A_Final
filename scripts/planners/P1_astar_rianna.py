...
class AStar(object):
    ...
    def is_free(self, x):
        ...
        if (x[0] < self.statespace_lo[0]) or (x[1] < self.statespace_lo[1]):
            return False
        if (x[1] > self.statespace_hi[0]) or (x[1] > self.statespace_hi[0]):
            return False
        return True
    ...
    def get_neighbors(self, x):
        ...
        ul = (x[0]-self.resolution, x[1]+self.resolution)
        ur = (x[0]+self.resolution, x[1]+self.resolution)
        dl = (x[0]-self.resolution, x[1]-self.resolution)
        dr = (x[0]+self.resolution, x[1]-self.resolution)

        # append if neighbor is free
        for n in [up, down, left, right, ul, ur, dl, dr]:
            if self.is_free(self.snap_to_grid(n)):
                neighbors.append(n)
        ########## Code ends here ##########
        return neighbors
    ...
    def solve(self):
        ...
        while len(self.open_set) > 0:
            ...
            for n in self.get_neighbors(x_curr):
                if n in self.closed_set:
                    continue
                tent_coa = self.cost_to_arrive[x_curr] + self.distance(x_curr, n)
                if n not in self.open_set:
                    self.open_set.add(n)
                elif tent_coa > self.cost_to_arrive[n]:
                    continue
                self.came_from[n] = x_curr
                self.cost_to_arrive[n] = tent_coa
                self.est_cost_through[n] = tent_coa + self.distance(n, self.x_goal)
        print("FAILED HERE")        
        return False
        ########## Code ends here ##########
