...
class AStar(object):
    ...
    def is_free(self, x):
        ...
        y_i = x[1]
        x_i = x[0]
        if(x_i >= self.statespace_lo[0] and y_i >= self.statespace_lo[1] and
           x_i <= self.statespace_hi[0] and y_i <= self.statespace_hi[1]):
            return True if self.occupancy.is_free(x) else False
        return False
        # ######### Code ends here ##########
    ...
    def get_neighbors(self, x):
        ...
        x, y = x
        neighbors = []
        neighbors.append(self.snap_to_grid((x, y + self.resolution)))
        neighbors.append(self.snap_to_grid((x + self.resolution, y)))
        neighbors.append(self.snap_to_grid((x, y - self.resolution)))
        neighbors.append(self.snap_to_grid((x - self.resolution, y)))
        neighbors.append(self.snap_to_grid((x + self.resolution, y + self.resolution)))
        neighbors.append(self.snap_to_grid((x - self.resolution, y + self.resolution)))
        neighbors.append(self.snap_to_grid((x - self.resolution, y - self.resolution)))
        neighbors.append(self.snap_to_grid((x + self.resolution, y - self.resolution)))
    ...
    def solve(self):
        ...
        while(len(self.open_set) > 0):
            iter += 1
            if iter > 2000:
                rospy.loginfo("UNABLE TO MAP PATH")
                return False
            # rospy.loginfo("COMPUTING ASTAR")
            ...
            for neighbor in neighbors:
                if(neighbor in self.closed_set):
                    continue
                temp_cost = self.cost_to_arrive[x_current] + self.distance(x_current, neighbor)
                if neighbor not in self.open_set:
                    self.open_set.add(neighbor)
                elif(temp_cost > self.cost_to_arrive[neighbor]):
                    continue
                self.came_from[neighbor] = x_current
                self.cost_to_arrive[neighbor] = temp_cost
                self.est_cost_through[neighbor] = temp_cost + self.distance(neighbor, self.x_goal)
        rospy.loginfo("FAILED  HERE")
        return False
        # ######### Code ends here ##########
