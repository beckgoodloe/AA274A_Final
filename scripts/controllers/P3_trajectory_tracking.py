...
class TrajectoryTracker:
    ...
    def compute_control(self, x, y, th, t):
        ...
        # V = 0
        # om = 0
        # u1 = xdd_d + self.kpx * (x_d - x) + self.kdx * (xd_d - self.V_prev*np.cos(th)*dt)
        # u2 = ydd_d + self.kpy * (y_d - y) + self.kdy * (yd_d - self.V_prev*np.sin(th)*dt)
        # J = np.array([[np.cos(th), -self.V_prev * np.sin(th)], [np.sin(th), self.V_prev * np.cos(th)]])
        # a, om = np.linalg.solve(J, [u1, u2])
