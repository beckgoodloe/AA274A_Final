...
class Ekf(object):
    ...
    def transition_update(self, u, dt):
        ...
        self.x = g
        self.Sigma = np.dot(Gx, np.dot(self.Sigma, np.transpose(Gx))) + dt * np.dot(Gu, np.dot(self.R, np.transpose(Gu)))
    ...
    def measurement_update(self, z_raw, Q_raw):
        ...
        ########## Code starts here ##########
        # TODO: Update self.x, self.Sigma.
       	S_t=np.matmul(H,np.matmul(self.Sigma,H.T))+Q
	K_t=np.matmul(self.Sigma,np.matmul(H.T,np.linalg.inv(S_t)))
	self.x=self.x+np.matmul(K_t,z)
	self.Sigma=self.Sigma-np.matmul(K_t,np.matmul(S_t,K_t.T))
        ########## Code ends here ##########
...
class EkfSlam(Ekf):
    ...
    def transition_model(self, u, dt):
        ...
        g_temp, Gx_temp, Gu_temp = tb.compute_dynamics(self.x[0:3], u, dt)
        Gx[:Gx_temp.shape[0],:Gx_temp.shape[1]]=Gx_temp
        Gu[:Gu_temp.shape[0],:Gu_temp.shape[1]]=Gu_temp
        g[:g_temp.shape[0]]=g_temp
    ...
    def compute_predicted_measurements(self):
        ...
        for j in range(J):
            ...
            h, Hx_temp = tb.transform_line_to_scanner_frame(np.array([alpha, r]), self.x[0:3], self.tf_base_to_camera)
            Hx[:, 0:3] = Hx_temp
            ...
            if j >= 2:
                ...
                x, y, th = self.x[0:3]
                dr_da = np.sin(alpha) * (x + np.cos(th_off)*x_off - np.sin(th_off)*y_off) - np.cos(alpha) * (y + np.sin(th_off) * x_off + np.cos(th_off)*y_off)
                dr_dr = 1.
                Hx[:,idx_j:idx_j+2] = np.array([[da_da, da_dr], [dr_da, dr_dr]])  # FIX ME!
            ########## Code ends here ##########
